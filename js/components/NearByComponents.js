...
import { createHotelIcon } from './MapComponents';
...
const NearByComponents = ({ hotels, onSelectHotel, setViewMode, isActive, onToast, onLocationUpdate }) => {
    ...
    useEffect(() => {
        ...
        nearbyHotels.forEach(hotel => {
            ...
            const marker = window.L.marker([hotel.lat, hotel.lng], { icon: hotelIcon });
            marker.on('click', (e) => {
            ...
            <div className="flex-1 w-full h-full relative z-0 pb-safe">
                ...
                <div className="absolute bottom-28 right-4 z-[1000] pointer-events-none">
                    <button 
                        ...
                        className="w-12 h-12 bg-white text-stone-600 rounded-full flex items-center justify-center shadow-[0_8px_30px_rgba(0,0,0,0.15)] border border-stone-100 pointer-events-auto cursor-pointer hover:text-blue-600 hover:bg-stone-50 active:scale-95 transition-all group"
